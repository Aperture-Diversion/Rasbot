...
def answer_generic():
    try: # DATE STUFF
        ...
        # TWEET IT
        twitter.update_status(status=head + replies_generic[number], in_reply_to_status_id=id)
...
def answer_how_are_you():
    try: # TWEET IT
        twitter.update_status(status=head + replies_feel[number], in_reply_to_status_id=id)
